    ...
    try{
        ...
        //prepare image upload
            ...
            if(!move_uploaded_file($image['tmp_name'], $targetPath)){
                ...
                $create_post_query = "INSERT INTO tbl_blog (id, author, title, sub_title, text, date, image) VALUES (NULL, :author, :title, :sub_title, :text, :date, :image);";
...
function getAllPosts(){
    ...
    $get_post_query = 'SELECT * FROM tbl_blog ORDER BY id DESC';
...
function previewPosts(){
    ...
    $preview_post_query = 'SELECT * FROM tbl_blog ORDER BY id DESC LIMIT 3';
...
function getPost($id){
    ...
    $get_post_query = "SELECT * FROM tbl_blog WHERE id = $id";
    $res = $pdo->query($get_post_query);
...
function editPost($id, $author, $title, $sub_title, $text, $date){
    ...
    $edit_post_query = 'UPDATE tbl_blog SET author = :author, title = :title, sub_title = :sub_title, text = :text, date = :date WHERE id = :id';
...
function deletePost($id){
    ...
    $delete_post_query = "DELETE FROM tbl_blog WHERE id = $id";
